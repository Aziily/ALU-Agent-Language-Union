...
3. Each `code` body MUST start with `def <node_name>(...):` (or `@decorator` then `def`, or an `# inject-into: <path>` comment immediately followed by the def). The function name MUST match the agent-lang node name UNLESS the node name uses `<Class>__<method>` form (in which case the function name is the method portion — see the "class method" pattern in the authoring guide). **If a body starts with `# inject-into: <path>`, keep that comment line verbatim** — it tells the harness which source file to patch, especially important when multiple files declare a function with the same name.

   **`# dangling-name: append-if-missing` marker (H12, Round 5)**: some bodies also carry a second comment ``# dangling-name: append-if-missing``. This signals "the source file referenced this function name (in a class body, in another module's `from X import Y`, etc.) but commit0 removed the def entirely — your job is to reconstruct it from context". The harness will APPEND your new def to the hinted file (it's not replacing a stripped stub since there isn't one). Keep both marker lines verbatim, and write a real implementation in the body — e.g. for `# dangling-name` `def raises(...)` in `voluptuous/schema_builder.py`, the function should be a context manager that asserts a specific Invalid is raised, because the existing docstrings reference it as `with raises(MultipleInvalid, ...): ...`.
